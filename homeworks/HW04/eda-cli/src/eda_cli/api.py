...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт для оценки качества данных из CSV-файла с возвратом полного набора флагов.
    """
    start = perf_counter()
    ...
    too_few_rows = flags_all.get("too_few_rows")
    too_many_columns = flags_all.get("too_many_columns")
    max_missing_share = flags_all.get("max_missing_share")
    too_many_missing = flags_all.get("too_many_missing")
    has_constant_columns = flags_all.get("has_constant_columns")
    constant_columns = flags_all.get("constant_columns")
    has_high_cardinality_categoricals = flags_all.get("has_high_cardinality_categoricals")
    high_cardinality_columns = flags_all.get("high_cardinality_columns")
    has_suspicious_id_duplicates = flags_all.get("has_suspicious_id_duplicates")
    id_duplicates_info = flags_all.get("id_duplicates_info")
    has_many_zero_values = flags_all.get("has_many_zero_values")
    many_zero_columns = flags_all.get("many_zero_columns")

    # Извлекаем score из флагов
    score = float(flags_all.get("quality_score", 0.0))
    score = max(0.0, min(1.0, score))
    ...
    # Возвращаем полный набор флагов (включая небулевые)
    return QualityFlagsResponse(
        latency_ms=latency_ms,
        flags={"too_few_rows": too_few_rows, 
               ...
               "has_suspicious_id_duplicates": has_suspicious_id_duplicates,
               "id_duplicates_info": id_duplicates_info,
               "has_many_zero_values": has_many_zero_values,
               "many_zero_columns": many_zero_columns,
               "score": score},  # Возвращаем все флаги, а не только булевые
        dataset_shape={"n_rows": n_rows, "n_cols": n_cols},
    )
