...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    HIGH_CARDINALITY = summary.n_rows * 0.9  # Более 90% уникальных значений плохо
    for col in summary.columns:
        # Проверяем нечисловые колонки
        if not col.is_numeric and col.unique > HIGH_CARDINALITY:
            has_high_cardinality_categoricals = True
            high_cardinality_cols.append({
                "name": col.name,
                "unique": col.unique,
                "threshold": HIGH_CARDINALITY
            })
    ...
    # 3 Проверка на дубликаты ID
    has_suspicious_id_duplicates = False
    id_duplicates_info = []
    for col in summary.columns:
        col_name_lower = col.name.lower()
        if 'id' in col_name_lower:
            # Для ID колонок ожидаем, что все значения уникальны
            if col.unique < col.non_null and col.non_null > 0:
                has_suspicious_id_duplicates = True
                duplicate_rate = 1.0 - (col.unique / col.non_null)
                id_duplicates_info.append({
                    "name": col.name,
                    "unique": col.unique,
    ...
    flags["id_duplicates_info"] = id_duplicates_info

    # 4 Проверка на много нулевых значений в числовых колонках
    has_many_zero_values = False
    many_zero_cols = []
    ZERO_VALUES = 0.5  # Более 50% нулевых значений плохо
    for col in summary.columns:
        if col.is_numeric and col.non_null > 0: # Для числовых колонок проверяем минимальное значение
            # Если min и max равны 0, скорее всего все значения нулевые
            if col.min == 0 and col.max == 0:
                zero_share = 1.0
                if zero_share > ZERO_VALUES:
                    has_many_zero_values = True
                    many_zero_cols.append({
                        "name": col.name,
                        "zero_share": zero_share,
                        "threshold": ZERO_VALUES
                    })
    ...
    if has_suspicious_id_duplicates:
        score -= 0.2  # более серьезная проблема
